...
export const Dashboard: React.FC = () => {
  ...
  const dataRangeText = useMemo(() => {
    if (heatingData.length === 0) return '';
    const first = heatingData[0];
    const last = heatingData[heatingData.length - 1];
    const left = `${first?.date ?? ''} ${first?.time ?? ''}`.trim();
    const right = `${last?.date ?? ''} ${last?.time ?? ''}`.trim();
    return `${left} - ${right}`;
  }, [heatingData]);

  const hasData = heatingData.length > 0;
            ...
            <div className="bg-gradient-to-r from-blue-500 to-indigo-600 rounded-xl p-6 text-white">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="opacity-90">
                      {t('dashboard.analyzingPoints')} {heatingData.length}{' '}
                      {t('dashboard.dataPoints')} (DB: {dataCount})
                    </p>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <p className="font-semibold">{dataRangeText}</p>
                </div>
              </div>
            </div>
          </div>
